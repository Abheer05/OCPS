...
class fun
{
   ...
   public function addCompany($name,$des,$min_ssc,$min_hsc,$min_cgpa,$skills,$date){
     $sql = "INSERT INTO `company`( `name`,`description`,`skills`,`min_hsc`,`min_ssc`,`min_cgpa`,`date_of_arrival`) VALUES ('$name','$des','$skills','$min_hsc','$min_ssc','$min_cgpa','$date')";
     $fetch = mysqli_query($this->db, $sql);
     return $fetch;
   }
   ...
   public function fetchSectionWithId($id){
    ...
    public function addCoord($name, $dept){
     $sql = "INSERT INTO `tandp`(`name`, `dept`) VALUES ('$name','$dept')";
     $fetch = mysqli_query($this->db,$sql);
     if($fetch){
          $this->addTandPUser($name);
     }
     return $fetch;
    }

    public function addTandPUser($name){
     $sql = "INSERT INTO `user`(`userid`, `pass`, `role`) VALUES ('$name@jit','tandp@jit','Training and Placement')";
     $fetch = mysqli_query($this->db,$sql);
     return $fetch;
    }

    public function fetchCordAll(){
     $sql = "SELECT * FROM `tandp`";
     $fetch = mysqli_query($this->db,$sql);
     return $fetch;
    }
    public function fetchCordWithId($id){
     $sql = "SELECT * FROM `tandp` where `id` = '$id'";
     $fetch = mysqli_query($this->db,$sql);
     return $fetch;
    }

    public function updateCoord($id, $name, $dept){
     $sql = "UPDATE `tandp` SET `name` = '$name', `dept` = '$dept' WHERE `tandp`.`id` = $id ";
     $fetch = mysqli_query($this->db,$sql);
     return $fetch;
    }

    public function deleteCoord($id){
     $sql = "DELETE FROM `tandp` WHERE `tandp`.`id` = $id";
     $fetch = mysqli_query($this->db,$sql);
     return $fetch;
    }
    ...
    public function fetchMatchingStudent($ssc,$hsc,$cgpa){
     $sql = "SELECT * FROM `student`
     INNER JOIN student_cgpa on student.id = student_cgpa.student_id 
     WHERE student.ssc_result>$ssc and student.hsc_result >$hsc and student_cgpa.total > $cgpa and student_cgpa.verify = 1";
     $fetch = mysqli_query($this->db,$sql);
     return $fetch;
    }
