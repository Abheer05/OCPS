...
class fun
{
   ...
   public function fetchSectionWithId($id){
    ...
    public function addClearanceCheck( $sid,$course_id,$tid,$clear ){
     $check = $this->checkClearanceById($sid,$course_id);
     if($check){
          $sql = "UPDATE `clearance_check` SET `clearance`='$clear' WHERE `sid`='$sid' and `course_id`='$course_id'";
     }
     else{

          $sql = "INSERT INTO `clearance_check`(`sid`, `course_id`, `tid`, `clearance`) VALUES ('$sid','$course_id','$tid','$clear')";
     }
     $result = mysqli_query($this->db,$sql);
     return $result;
    }

    public function fetchClearanceCheckWithId($sid,$course_id){
    ...
    public function checkClearanceById( $sid,$course_id ){
     $sql = "SELECT * FROM `clearance_check` WHERE `sid` = '$sid' and `course_id` = '$course_id'";
     $result = mysqli_query($this->db,$sql);
